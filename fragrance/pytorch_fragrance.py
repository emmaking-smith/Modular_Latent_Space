'''
Creating the Dataset for the DataLoader for Fragrance Predictions.
...
class Odor_Classification(Dataset):
    def __init__(self, df, longest_molecule):
        super(Odor_Classification, self).__init__()
    ...
    def extract_fragrance_class(self, df_row):
    ...
    def __getitem__(self, index):
        ...
        fragrance_classes = torch.tensor(fragrance_classes).float()

        return (mol_features, mol_matrices), (fragrance_classes)

def collate_fn(batch):
    batch_size = len(batch)
    mol_features, mol_matrices = batch[0][0]
    fragrance_classes = batch[0][1]
    ...
    for i in range(1, batch_size):
        ...
        fragrance_classes = torch.cat([fragrance_classes, batch[i][1]], 0)
    ...
    fragrance_classes = fragrance_classes.view([batch_size, -1])
    return mol_features, mol_matrices, fragrance_classes