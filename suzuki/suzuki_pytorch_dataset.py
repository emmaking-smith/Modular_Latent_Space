...
from suzuki_splits import find_reactants, find_catalysts_ligands, canonicalize_smiles
...
class Suzuki_USPTO_Rxns(Dataset):
    def __init__(self, rxn_dict, longest_boronic_acid, longest_halide, longest_catalyst, longest_ligand):
        super(Suzuki_USPTO_Rxns, self).__init__()

        self.rxn_dict = rxn_dict
        self.longest_boronic_acid = longest_boronic_acid
        self.longest_halide = longest_halide
        self.longest_catalyst = longest_catalyst
        self.longest_ligand = longest_ligand

    def __len__(self):
        return len(self.rxn_dict)
    ...
    def __getitem__(self, index):
        index = list(self.rxn_dict.keys())[index]
        # Get the canonical smiles of boronic acid, halide, catalyst, and ligand.
        boronic_acid, halide = find_reactants(self.rxn_dict[index]['rx'])
        catalysts, ligands = find_catalysts_ligands(self.rxn_dict[index])
        ...
        boronic_acid_features, boronic_acid_adj_matrices = self.single_molecule_matrices_and_features_workflow(boronic_acid,
                                                                                                               self.longest_boronic_acid)
        halide_features, halide_adj_matrices = self.single_molecule_matrices_and_features_workflow(halide,
                                                                                                   self.longest_halide)
        catalyst_features, catalyst_adj_matrices = self.single_molecule_matrices_and_features_workflow(catalysts, self.longest_catalyst)

        ligand_features, ligand_adj_matrices = self.single_molecule_matrices_and_features_workflow(ligands, self.longest_ligand)

        yields = self.rxn_dict[index]['yield']

        halide_features = torch.tensor(halide_features).float()
        halide_adj_matrices = torch.tensor(halide_adj_matrices).float()
        boronic_acid_features = torch.tensor(boronic_acid_features).float()
        boronic_acid_adj_matrices = torch.tensor(boronic_acid_adj_matrices).float()
        catalyst_features = torch.tensor(catalyst_features).float()
        catalyst_adj_matrices = torch.tensor(catalyst_adj_matrices).float()
        ...
        return (halide_features, halide_adj_matrices, boronic_acid_features, boronic_acid_adj_matrices,
                catalyst_features, catalyst_adj_matrices, ligand_features, ligand_adj_matrices), (yields)

def collate_fn(batch):
    batch_size = len(batch)
    halide_features, halide_adj_matrices, boronic_acid_features, boronic_acid_adj_matrices, catalyst_features, catalyst_adj_matrices, ligand_features, ligand_adj_matrices = batch[0][0]
    yields = batch[0][1]

    longest_halide = halide_adj_matrices.size()[-1]
    longest_boronic_acid = boronic_acid_adj_matrices.size()[-1]
    longest_catalyst = catalyst_adj_matrices.size()[-1]
    ...
    for i in range(1, batch_size):
        ...
        boronic_acid_features = torch.cat([boronic_acid_features, batch[i][0][2]], 0)
        boronic_acid_adj_matrices = torch.cat([boronic_acid_adj_matrices, batch[i][0][3]], 0)
        catalyst_features = torch.cat([catalyst_features, batch[i][0][4]], 0)
        catalyst_adj_matrices = torch.cat([catalyst_adj_matrices, batch[i][0][5]], 0)
        ligand_features = torch.cat([ligand_features, batch[i][0][6]], 0)
        ligand_adj_matrices = torch.cat([ligand_adj_matrices, batch[i][0][7]], 0)
    ...
    boronic_acid_features = boronic_acid_features.view([batch_size, longest_boronic_acid, -1])
    boronic_acid_adj_matrices = boronic_acid_adj_matrices.view([batch_size, -1, longest_boronic_acid, longest_boronic_acid])
    catalyst_features = catalyst_features.view([batch_size, longest_catalyst, -1])
    catalyst_adj_matrices = catalyst_adj_matrices.view([batch_size, -1, longest_catalyst, longest_catalyst])
    ...
    return halide_features, halide_adj_matrices, boronic_acid_features, boronic_acid_adj_matrices, catalyst_features, catalyst_adj_matrices, ligand_features, ligand_adj_matrices, yields